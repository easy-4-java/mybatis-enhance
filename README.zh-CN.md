...
mybatis-enhance 是面向**原生 MyBatis** 的增强组件。
...
跨版本线同步以能力和公共契约一比一为目标，但依赖版本、编译配置、JSqlParser API 适配与 Java 语法必须分别符合对应 JDK 基线，不能机械复制构建配置（见 [COMPATIBILITY.md](COMPATIBILITY.md)）。本分支快照版本为 `1.0.x.20260630-SNAPSHOT`；正式版本通过 GitHub Releases 发布，Maven Central 发布已规划，尚未执行。
